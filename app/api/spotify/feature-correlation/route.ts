...
export async function GET(req: NextRequest) {
  // 1) Authenticate and get Spotify access token
  const token = await getToken({ req, secret: process.env.NEXTAUTH_SECRET });
  if (!token?.accessToken) {
    return NextResponse.json({ error: "Not authenticated" }, { status: 401 });
  }

  // 2) Determine the time range (short_term, medium_term, long_term)
  const url = new URL(req.url);
  const time_range = url.searchParams.get("time_range") ?? "medium_term";

  // 3) Fetch your top 50 tracks for that period
  ...
  // 5) Define which numeric features to correlate
  const keys = [
    "danceability",
    "energy",
    "valence",
  ...
  // 6) Compute mean for each feature
  const mean: Record<string, number> = {};
  for (const k of keys) {
    mean[k] =
      feats.reduce((sum: number, f: any) => sum + (f[k] || 0), 0) / feats.length;
  }

  // 7) Build the correlation matrix
  const matrix: number[][] = keys.map((xi) =>
    keys.map((xj) => {
      let cov = 0,
        varx = 0,
        vary = 0;
